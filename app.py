...
# 役職リスト（よくある役職名を追加）
job_titles = ["代表", "取締役", "部長", "社長", "専務", "理事", "監査役", "役員", "議員", "審議官", "教授", "会長", "研究員"]
...
def process_articles(df, keywords, column_name):
    ...
    # 名詞・動詞・人名 + 役職の割合を計算
    filtered_df[['noun_ratio', 'verb_ratio', 'people_job_sentence_ratio']] = filtered_df[column_name].apply(lambda x: pd.Series(calculate_text_ratios(str(x))))
